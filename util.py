...
#The following functions are copied from from https://github.com/tkipf/gcn/blob/master/gcn/utils.py
...
#the following function is copied from https://github.com/yincheng/GGP/blob/master/ggp/utils.py
def load_data_ssl(data_name):
    adj_csr, features, y_train, y_val, y_test, train_mask, val_mask, test_mask = load_data(data_name)
    adj_mat = np.asarray(adj_csr.toarray(), dtype=np.float64)
...
#a method that loads data from load_data_ssl and changes format so it can be used with the custom kernel
def load_data_BA(data_name):
    adj, features, x_train, y_train, x_val, y_val, x_test, y_test =load_data_ssl(data_name)
    ...
    label_number=np.max(y_train)+1
    y_train=y_train.astype('float64')
    x_train=x_train.astype('float64')
    y_test=y_test.astype('float64')
    x_test=x_test.astype('float64')
    features=features.astype('float64')
    return adj, features, x_train,x_test,y_train,y_test,label_number
