#the following code is adapted from https://github.com/GPflow/GPflow/blob/develop/gpflow/models/svgp.py
...
from ggp_conditional import conditional_train
...
class SVGGP(GPModel, ExternalDataTrainingLossMixin):
    """
    ...
    def elbo(self, data: RegressionData) -> tf.Tensor:
        ...
        f_mean, f_var = self.predict_f_train(X, full_cov=False, full_output_cov=False) #use special predict f_train to speed up performance
    ...
    def predict_f_train(self, Xnew: InputData, full_cov=False, full_output_cov=False) -> MeanAndVariance:
        q_mu = self.q_mu
        q_sqrt = self.q_sqrt
        mu, var = conditional_train(
            Xnew,
